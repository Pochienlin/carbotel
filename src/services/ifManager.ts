...
import { Interpolation } from '@grnsft/if/build/if-run/builtins/interpolation';
import { IMPInputs, PluginParams } from '../common/types';
import { PluginParametersMetadata, MappingParams } from '@grnsft/if-core/types';
...
const parametersMetadata = {} as PluginParametersMetadata;
...
// ;
const interpolationPlugin_CPU = Interpolation(
  config_cpu,
  parametersMetadata,
  {} as MappingParams,
);

async function runProcessorInterpolation(data: PluginParams[]) {
  try {
...
export async function runIFbyMaps(inputs: IMPInputs) {
  ...
  return output
    .map((e) => ({
      ...
      memEnergy:
        (((inputs.memCoeff * e.memory) / 1000000000) * e.duration) / 3600,
